...
namespace writeexcel\functions;
 ...
 * A1 notation.
 */
function xlRowcolToCell( $row, $col, $row_abs = false, $col_abs = false )
{
    $row_abs = $row_abs ? '$' : '';
    $col_abs = $col_abs ? '$' : '';
 ...
 *
 * @return: array($row, $col, $row_absolute, $col_absolute)
 *
 ...
 *
 * @return: a cell reference string in A1 notation.
 */
function xlIncRow( $cell )
{
    list($row, $col, $row_abs, $col_abs) = xlCellToRowcol( $cell );

    return xlRowcolToCell( ++$row, $col, $row_abs, $col_abs );
}
 ...
 * @return: a cell reference string in A1 notation.
 */
function xlDecCol( $cell )
{
    list($row, $col, $row_abs, $col_abs) = xlCellToRowcol( $cell );
    return xlRowcolToCell( $row, --$col, $row_abs, $col_abs );
}

function xlDateList( $year, $month = 1, $day = 1, $hour = 0, $minute = 0, $second = 0 )
{
...
function xlParseDate( $date )
{
    ...
    return xlDateList( $year, $month, $day, $hour, $minute, $second );
}

/**
 * Dummy function to be "compatible" to Spreadsheet::WriteExcel
 */
function xlParseDateInit()
{
    // Erm... do nothing...
 ...
 */
function xlDecodeDateEU( $date )
{
    return xlParseDate( $date );
}

function xlDecodeDateUS( $date )
{
    return xlParseDate( $date );
}

function xlDate1904( $exceldate )
{
    if( $exceldate < 1462 )
    {
        // date is before 1904
